...
*******************************************************************************/
uint32_t USB_SIL_Read(uint8_t bEpAddr, uint8_t* pBufferPointer, uint16_t nBufferLen, uint32_t *TotalLength, uint32_t bufferOffset)
{
  ...
  DataLength = GetEPRxCount(bEpAddr & 0x7F) - bufferOffset;
  *TotalLength = DataLength;
  
  if(nBufferLen < DataLength)
	  DataLength = nBufferLen;

  /* Use the memory interface function to write to the selected endpoint */
  PMAToUserBufferCopy(pBufferPointer, GetEPRxAddr(bEpAddr & 0x7F) + bufferOffset, DataLength);
